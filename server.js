...
const MAIL_TO = process.env.MAIL_TO || "haboub.m@hotmail.com";
...
const DEFAULT_CONTENT = {
  ...
  hero: {
    ...
    lead: "Från totalrenoveringar till badrum, kök och tillbyggnader — NordBygg levererar hantverk du kan lita på, i tid och inom budget.",
    stats: [
  ...
  contact: {
    ...
    email: "haboub.m@hotmail.com",
    address: "Andersbergsringen 103, 302 55 Halmstad",
...
app.get("/sitemap.xml", (req, res) => {
  const base = process.env.SITE_URL || "https://www.nordbygg.se";
...
async function sendMail({ subject, replyTo, fields, kind }) {
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:auto">
      <div style="background:#ffc107;padding:18px 22px;border-radius:12px 12px 0 0">
        <h2 style="margin:0;color:#14161a;font-size:18px">NordBygg — ny ${kind}</h2>
      </div>
      ...
      <p style="color:#9aa0ab;font-size:12px;margin-top:14px">Skickat från nordbygg-webbplatsen den ${new Date().toLocaleString(
        "sv-SE"
      )}.</p>
    </div>`;
  const text = fields.filter((f) => f.value).map((f) => `${f.label}: ${f.value}`).join("\n");
  return transporter.sendMail({
    from: `"NordBygg webbformulär" <${process.env.SMTP_FROM || process.env.SMTP_USER}>`,
    to: MAIL_TO,
...
app.listen(PORT, () => {
  console.log(`NordBygg körs på http://localhost:${PORT}`);
